  ...
  if (
    ...
    return res
      ...
      .json({
        success: false,
        message:
          "Invalid request. Please provide customer details and selections.",
      });
  }
  ...
  try {
    ...
    for (const selection of selections) {
      ...
      if (!meal_date || !meal_type) {
        throw new Error(
          "Invalid selection format. Each selection must have a meal_date and meal_type."
        );
      }

      const dayOfWeek = new Date(meal_date).toLocaleString("en-US", {
        weekday: "long",
      });
      ...
      if (!menuItem) {
        throw new Error(
          `The selected meal (${meal_type} on ${meal_date}) is not available.`
        );
      }
    ...
    res.status(500).json({
      success: false,
      message:
        error.message || "Failed to initiate order due to a server error.",
    });
  }
};
const updateNumberofCoupons = async (day_of_week, meal_type) => {
  const menuItem = await MenuItem.findOne({
    where: { day_of_week: day_of_week, meal_type: meal_type },
  });
  if (!menuItem) {
